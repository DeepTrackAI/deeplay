...
class RNN(DeeplayModule):
    ...
    def __init__(
        ...
        self.rnn.blocks.dropout.configure(torch.nn.Dropout, p=dropout)
    ...
    def forward(self, x, lengths: Optional[torch.Tensor] = None):
        ...
        for block in self.blocks[:-1]:
             outputs, hidden = block.layer[0](outputs)
             outputs = block.activation[0](outputs)
             outputs=block.normalization[0](outputs)
             outputs=block.dropout[0](outputs)

        if not self.out_features:
             outputs, hidden = self.blocks[-1].layer[0](outputs)
             outputs = self.blocks[-1].activation[0](outputs)
             outputs=self.blocks[-1].normalization[0](outputs)

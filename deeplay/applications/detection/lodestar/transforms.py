...
class RandomTranslation2d(Transform):
    ...
    def __init__(self, 
                 dx=lambda: np.random.uniform(-1, 1), 
                 dy=lambda: np.random.uniform(-1, 1), 
                 indices=(0, 1)):
    ...
    def _forward(x, dx, dy, indices):
        ...
        return kornia.geometry.transform.translate(x, translation, align_corners=True, padding_mode="reflection")
...
class RandomRotation2d(Transform):
    ...
    def _forward(x, angle, indices):
        ...
        return kornia.geometry.transform.rotate(x, angle * 180 / np.pi, align_corners=True, padding_mode="reflection")
    
    @staticmethod
    def _backward(x, angle, indices):
        mat2d = torch.eye(x.size(1), device=x.device).unsqueeze(0).repeat(x.size(0), 1, 1)

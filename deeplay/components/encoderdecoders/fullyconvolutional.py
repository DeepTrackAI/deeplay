...
def convolutionalEncoderDecoderConfig():
    return (
        ...
        .encoder_blocks.layer(nn.Conv2d, kernel_size=3, padding=1)
        ...
        .decoder_blocks.layer(nn.Conv2d, kernel_size=3, padding=1)
        .decoder_blocks.populate("layer.out_channels", lambda i: 8 * 2**(4 - i), length=8)
        .decoder_blocks.activation(nn.ReLU)
        .decoder_blocks.upsample(nn.Upsample, scale_factor=2, mode="bilinear", align_corners=True)
    )
...
class ConvolutionalEncoderDecoder(EncoderDecoder):
    ...
    @staticmethod
    def defaults():
        return fullyConvolutionalEncoderDecoderConfig()
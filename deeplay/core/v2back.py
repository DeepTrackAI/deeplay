    ...
    def __new__(cls, name, bases, attrs):
        ...
        for attr_name, attr in attrs.items():
            if name.startswith("_"):
                continue
            if isinstance(attr, _DeeplayModuleBase):
...
class DeeplayModule(_DeeplayModuleBase, metaclass=CreateNewModuleOnInitMeta):
    ...
    def __new__(cls, *args, **kwargs):
        ...
        obj._class_config = Config()  # type: ignore
        obj._user_config = Config()  # type: ignore
        return obj
    ...
    def __init__(self, *args, **kwargs):
        ...
        for key, value in self._class_modules.items():
            ...
            value = value.new()
            for method_name in self.__builder_functions__:
    ...
    def build(self) -> "DeeplayModule":
        ...
        config_items = self.config.get_parameters(create=True).items()
        named_children = self.named_children()

        both = {}
        both.update(config_items)
        both.update(named_children)
...
class layerlist(NamedModuleMixin, nn.ModuleList):
...
class LayerList(DeeplayModule, nn.ModuleList):
    ...
    def build(self, builder=None):
        for idx in range(len(self)):
            if self._builder_func is not None:
                layer = self._builder_func(builder, idx)
            else:
                layer = self[idx]
